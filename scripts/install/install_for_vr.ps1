# REFramework VR Installation Script
# For RE7/RE8 VR mod installation

param(
    [string]$GamePath,
    [string]$Game = "RE7",
    [switch]$UseOpenXR = $true,
    [switch]$Force,
...
function Install-VR($ZipPath, $GamePath, $UseOpenXR) {
    Write-Status "Installing VR mod to: $GamePath"
    ...
    foreach ($file in $files) {
        # Skip openvr_api.dll if using OpenXR
        if ($UseOpenXR -and $file.Name -eq "openvr_api.dll") {
            Write-Status "Skipping openvr_api.dll (using OpenXR)" "Info"
            continue
        }

        $destPath = Join-Path $GamePath $file.Name
        Copy-Item -Path $file.FullName -Destination $destPath -Force
        Write-Status "Copied: $($file.Name)" "Success"
    }

    Write-Status "VR installation complete!" "Success"
}

# Main execution
Write-Host ""
Write-Host "========================================" -ForegroundColor Magenta
Write-Host "  REFramework VR Installer" -ForegroundColor Magenta
Write-Host "========================================" -ForegroundColor Magenta
Write-Host ""
...
Install-VR -ZipPath $zipPath -GamePath $GamePath -UseOpenXR $UseOpenXR
...
Write-Host "========================================" -ForegroundColor Green
Write-Host ""
Write-Status "Tips for RE7 VR:" "Info"
Write-Host "  - Set Ambient Occlusion to SSAO or Off"
Write-Host "  - Use DirectX 11 (non-RT) version for stability"
